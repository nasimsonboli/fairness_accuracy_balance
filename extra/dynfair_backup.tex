...
\newcommand{\nick}[1]{\todo[color=kentuckyblue!40]{Nick says: #1}}
\newcommand{\nn}[1]{\textbf{\color{kentuckyblue}{NICK TEXT: #1}}}
\newcommand{\nasim}[1]{\todo[color=green!40]{Nasim says: #1}}
\newcommand{\robin}[1]{\todo[color=yellow!40]{Robin says: #1}}
...
%%%% Conference Paper Macros
\newcommand{\ocmax}{\ensuremath{c^{O}_{max}}}
\newcommand{\ocmin}{\ensuremath{c^{O}_{min}}}
\newcommand{\acmax}{\ensuremath{c^{N}_{max}}}
\newcommand{\acmin}{\ensuremath{c^{N}_{min}}}

\newcommand{\reals}{\ensuremath{\mathbb{R}}}
\newcommand{\wrs}{\ensuremath{\omega}}
\newcommand{\owa}{\ensuremath{\alpha}}
\newcommand{\all}{\ensuremath{\mathbb{A}}}
\newcommand{\uv}{\ensuremath{\mathbf{u}}}
\newcommand{\xv}{\ensuremath{\mathbf{x}}}
\newcommand{\maxv}{\ensuremath{\Delta}}
\newcommand{\rev}{\ensuremath{\tau}}
\newcommand{\sig}{\ensuremath{\sigma}}
\newcommand{\sumowa}{$\Sigma$-OWA~}

\title{``And the Winner Is...'': Dynamic Lotteries for Multi-group Fairness-Aware Recommendation}

% SCRUFF: Social Choice for Re-ranking featUre Fairness
% SCRuFf: Social Choice for Re-ranking Fairness
% SCOFfeR: Social Choice for Opportunistic Fair Re-ranking
% SCORFF: Social Choice for Opportunistic Re-ranking of Feature Fairness

\author{Nasim Sonboli, Robin Burke}
\affiliation{%
...
In addition to the core property of accurate personalization -- delivering results that match user interests and preferences -- recommender systems may need to satisfy other, non-accuracy, constraints in certain applications. One property of interest that has received significant attention recently is \emph{fairness}: the constraint that a recommender system should distribute its benefits fairly across different stakeholder groups \cite{yao2017beyond,burke2018balanced,ekstrand2018exploring,liu2019personalized,kamishima2016model,beutel2019fairness}. For example, all else being equal, a job recommender system should not recommend executive jobs to male users and clerical jobs to female users.
...
\subsection{Computational Social Choice for Fair Recommendations}
...
\subsection{Recommendation Systems with Protected Values}
...
\subsection{Metrics for Fair Recommendation}
...
\subsection{User Preferences for Fairness}
...
% Liu et al. \cite{liu2018personalizing,liu2019personalized} introduced the concept of recommendation re-ranking using a quantity $\tau_u$, a user-specific measure of interest in diversity, based on information entropy.
...
\begin{figure*}[tb]
    \centering
    \includegraphics[width=5.5in]{img/process-graphic2.png}
...
\subsection{Sampling Lottery}
While the dynamic lottery does take into account the current state of the $M_j$ metrics, it operates under the assumption that we have an infinite number of user visits to achieve a fair result. To build a more effective dynamic lottery, we make use of the monotonicity property of the re-ranking functions, and treat the re-rankers as if they have a fixed and equal impact on their respective metrics. Under this assumption, fairness becomes a matter of equalizing the number of times that a re-ranker runs.\footnote{In future work, we will explore on-line learning of the relationship between re-ranking functions and fairness metrics to quantify the expected impact of applying a re-ranker.} 

We can re-cast the formulation of the lottery therefore as a distributional sampling problem. We are given some distribution $p$, which is different from target distribution $q$. You want to draw samples so that $p$ approaches $q$. From what distribution $p'$ should you draw each sample so $p \rightarrow q$ as quickly as possible (i.e., in the minimum number of steps)?  

\begin{theorem}
Given a backward-looking window of $w_b$ samples of a discrete distribution $p$ and an expected number of samples (forward window) $w_f$ to achieve a desired discrete distribution $q$, the new sampling distribution $p'$ must satisfy $ p_i' = \frac{ w_f q_i + w_b ( q_i - p_i)}{w_f}, \forall i \in \sensitive$, i.e., all $i$ in the support of $p$ since we are only considering the distribution over the sensitive attributes.   
\end{theorem}
\begin{proof}
$\forall i$ but the last $i$, it is desired that $\frac{w_b p_i + w_f p_i'}{w_f+w_b} = q_i$, hence $w_b * p_i + w_f * p_i'= q_i w_b + q_i * w_f$, which gives $ p_i' = \frac{ w_f * q_i + w_b q_i - w_b p_i}{w_f}$. 
\end{proof}

Note that it is possible for the numerator to be smaller than $0$ and  the resulting $p'$ is no longer a valid probability. We must therefore constrain $w_f$ to only certain values, depending on the values of $q_i$ and $p_i$. 

\begin{theorem}
The minimum $w_f$ to achieve a valid $p'$ is $w_f \geq \max_{i \in \sensitive}{\min (\frac{w_b q_i - w_b p_i}{1-q_i}}, \frac{w_b p_i - w_b q_i }{q_i} ) $. 
\end{theorem}

\begin{proof}
Since $0 \leq p_i' \leq 1$ must hold for $p'$ to be a valid distribution, $ 0 \leq \frac{ w_f q_i + w_b q_i - w_b p_i}{w_f} \leq 1$. Assume $w_f > 0$, to ensure $p_i \geq 0$, we have $ w_f q_i \geq w_b p_i - w_b q_i \Rightarrow w_f \geq \frac{ w_b p_i - w_b q_i}{q_i}$.

Similarly, to ensure $p_i \leq 1$, we have $w_b q_i - w_b p_i \leq w_f - w_f q_i \Rightarrow w_f \geq \frac{w_b q_i - w_b p_i}{1 - q_i}$.
\end{proof}

For the purposes of our present experiments, we are treating the target distribution as a uniform distribution, although we plan to explore other distributions in future work. Given a look-ahead window $w_f$, we can therefore compute a new $p^{\prime}$ distribution. The probability associated with each feature $j$ becomes the probability of choosing the associated re-ranker in the choice function $C$.

Note that we do not actually use the computed lottery for $w_f$ iterations. At each time step, we compute a new lottery with new probabilities for each re-ranker. The parameter $w_f$ determines how strong the response is to non-uniformity across the metrics. At low values of $w_f$ (near the theoretical minimum), the difference $w_b (q_i - p_i)$ will dominate the calculation. At higher values, $q_i$, the (fair) target distribution, will dominate and the model essentially becomes identical to the fixed lottery above.
...
% \nick{reveiwer comment: Regarding using rarely-recommended items as sensitive features and protected groups: Which features and groups were identified as protected?}
...
\setlength\tabcolsep{0pt}
    ...
    \centering
    \includegraphics[width=4.5in]{img/ml_avg_regret_overtime_sep20.png}
    \caption{MovieLens data set}
    \label{fig:local-regret-ML}
    \end{subfigure}
    \begin{subfigure}{1.0\textwidth}
    \centering
      \includegraphics[width=4.5in]{img/kiva_avg_regret_overtime_sep20.png}
    \caption{Kiva data set}
...
\begin{figure*}[t!]
    ...
    \begin{subfigure}[t]{0.5\textwidth}
        \centering
        \includegraphics[width=3.0in]{img/ML_regret_variance_sep20.png}
        \caption{MovieLens}
        \label{fig:local-regret-ML}
    \end{subfigure}%
    ~ 
    \begin{subfigure}[t]{0.5\textwidth}
        \centering
        \includegraphics[width=3.0in]{img/kiva_regret_variance_sep20.png}
        \caption{Kiva}
        \label{fig:local-regret-Kiva}
    \end{subfigure}
...
% \nick{Reviewer comment: Fig. 2 axes are not labelled.} 
