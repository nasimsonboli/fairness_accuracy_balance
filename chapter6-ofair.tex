...
% \todo[inline]{clean the writing of this section, unclear and messy}
...
\vspace{0.25cm}
\noindent \textbf{(\romannum{1})~Personalized Diversity}
\vspace{0.25cm}
...
\vspace{0.25cm}
\noindent \textbf{(\romannum{2})~Recommendation Re-ranking}
\vspace{0.25cm}
...
\subsection{Opportunistic Fairness}
...
\vspace{0.25cm}
\noindent \textbf{(\romannum{1})~Evaluation Metrics}
\vspace{0.25cm}
...
\vspace{0.25cm}
\noindent \textbf{(\romannum{2})~Dataset}
\vspace{0.25cm}
...
\vspace{0.25cm}
\noindent \textbf{(\romannum{3})~Variation in Diversity Tolerance}
\vspace{0.25cm}
...
\vspace{0.25cm}
\noindent \textbf{(\romannum{4})~Comparing Re-ranking Algorithms}
\vspace{0.25cm}
...
Figure \ref{fig:kiva_mmr_based_methods} shows the results on the Kiva dataset for just the MMR-based algorithms: MMR, OFiAR, and the two versions incorporating different aspects of the OFAiR algorithm, tolerance weights (users) only, and fairness weights (items) only. The figure compares ranking accuracy in the form of nDCG versus the average exposure for protected items across the recommendation lists. The figure gives a more complete picture of this tradeoff than the tables above, but generally tells the same story.
...
Significantly, OFAiR has a dominant position among the other algorithms in terms of the fairness/accuracy tradeoff when viewed across all items in the protected group.
...
% We intend to publish this work later this year in The ACM Series on Recommender Systems.
...
% We intend to explore this type of algorithm design and evaluation in our future work.